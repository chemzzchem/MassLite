...
def on_read_button_click():
    ...
    if file_imzML_path:
        ...
        p0 = read_msi_data(ms_file,thresh=noise_threshold,ppm=mass_threshold)
    
    #p0 = read_msi_data(ms_file, thresh = Entry_noise_thresh.get())
    timing_test("data read-in")
    label_status.config(text="Read!")
...
def on_combobox_select(event):
    data_type = int(combobox_class.get()[6])
...
combobox_class.grid(row=2,column=1)
