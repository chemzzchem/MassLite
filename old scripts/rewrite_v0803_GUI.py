...
#def locate_peaks(p,marker_range = [(782.54,782.58),(760.56,760.60),(734.55,734.59),(788.52,788.56)]):
    ...
    # ax1 = plt.bar(x=range(300),height=marker_i[:300],width=1.0)

    peaks, _ = find_peaks(marker_i,distance=5)
    lows, _ = find_peaks(marker_i*(-1),distance = 5)
    peak_regions = []
    for peak_index in peaks:
        peak_value = marker_i[peak_index]
        threshold = peak_value * 0.2
        left_index = peak_index
        right_index = peak_index

        while left_index > 0 and marker_i[left_index - 1] >= threshold:
            left_index -= 1
        while right_index < len(marker_i) - 1 and marker_i[right_index + 1] >= threshold:
            right_index += 1

        peak_regions.append(np.array((left_index,right_index)))

    peak_regions_no_overlap = merge_overlapping_ranges(peak_regions)
    
    print("done!")
    return peak_regions_no_overlap
...
file_path = '48h live_2.mzML'
#file_path = 'cali2_1.mzML'
