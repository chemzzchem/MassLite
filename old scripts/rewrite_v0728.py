...
def classify_scans(p0):
    ...
    peak_n_hist = np.zeros((len(p0),len(fixed_bin)-1))
    ...
    kmeans_one = KMeans(n_clusters=3, random_state=0, n_init="auto").fit(peak_n_hist)
    n_label = kmeans_one.labels_
    kmeans_two = KMeans(n_clusters=3, random_state=0, n_init="auto").fit(peak_int_hist)
    int_label = kmeans_two.labels_
    kmeans_three = KMeans(n_clusters=3, random_state=0, n_init="auto").fit(peak_log_int_hist)
    int_log_label = kmeans_three.labels_
