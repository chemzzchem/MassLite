...
def classify_scans(p0,ncl=3):
    ...
    kmeans = KMeans(n_clusters=ncl, random_state=0).fit(peak_log_int_hist)
...
def ms_alignment0(data = p0,ppm=5):
    # alignment with hierarchical clustering without grouping
...
def ms_alignment1(data = p0,ppm=5):
    # initiating data array
    pixels = p0
    total_rows = sum(array.shape[0] for array in pixels)
    data = np.zeros((total_rows,5))
    ...
    #basic alignment with dynamic grouping in chunks
    rela_diff = np.diff(data[:,3])
    tentative_chunks = np.where(rela_diff>3*ppm)[0]+1

    chunks = np.array_split(data,tentative_chunks)
...
def ms_alignment2(data = p0,ppm=5):
    # initiating data array
    pixels = p0
    total_rows = sum(array.shape[0] for array in pixels)
    data = np.zeros((total_rows,5))
    ...
    tentative_chunks = np.where(data[:,4]>3*ppm)[0]
    chunk_sizes = np.diff(tentative_chunks)
    while(chunk_sizes.max()>2*len(pixels)):
        tentative_chunks_new = np.copy(tentative_chunks)
        oversize_chunks = np.where(chunk_sizes>2*len(pixels))[0]
        for oversize_chunk in oversize_chunks:
            chunk_begin = tentative_chunks[oversize_chunk]
            chunk_end = tentative_chunks[oversize_chunk + 1]
            if np.max(data[chunk_begin+1:chunk_end,4])>2*ppm:
                tentative_chunks_new = np.append(tentative_chunks_new,np.argmax(data[chunk_begin+1:chunk_end,4])+chunk_begin+1)
        if len(tentative_chunks_new) == len(tentative_chunks): 
            print("chunk division completed")
            break
        else:
            print("chunk evolving")
            tentative_chunks = np.sort(tentative_chunks_new)
            chunk_sizes = np.diff(tentative_chunks)
...
def exp_org_table(p,exp_file_path,datatype = 'scan'):
    ...
    p_with_nan = np.full((max_rows,2*len(p)),np.nan)
...
def on_classify_button_click():
    ...
    group_label,group_int = classify_scans(p0,ncl=int(Entry_gn.get()))
...
button_MLC.grid(row=3,column=2)
combobox_class = ttk.Combobox(main_window,values=[])
combobox_class.grid(row=3,column=3,columnspan=4)
...
radio_in_scan.grid(row=10,column=8)
...
checkbox_al.grid(row = 11, column=8,sticky="w")
...
checkbox_fi.grid(row = 12, column=8,sticky="w")
...
checkbox_bg.grid(row = 13, column=8,columnspan=2,sticky="w")

separator_r_h_3 = ttk.Separator(main_window,orient='horizontal')
separator_r_h_3.grid(row=14,column=8,columnspan=4,sticky="ew", pady=10)

label_log = tk.Label(main_window,text='User Log')
label_log.grid(row=15,column=8)
button_debug = tk.Button(main_window,text='Debug',command=on_debug_button_click)
button_debug.grid(row=15, column=10)
listbox_log = tk.Listbox(main_window, width=80, height=12)
listbox_log.grid(row=16, column=8, columnspan=3,rowspan=5,padx=10, pady=10)
scrollbar_log = tk.Scrollbar(main_window, command=listbox_log.yview)
scrollbar_log.grid(row=16, column=11, pady=10, rowspan=5,sticky="ns")
...
timing_test("cell sorting")

# p_align_0 = ms_alignment0(p0,ppm=5)
timing_test("alignment 0")
p_align_b = ms_alignment(p0,ppm=5)
timing_test("Direct Chunk")
# p_align_1 = ms_alignment1(p0,ppm=5)
timing_test("Dynamic Grouping")
p_align_2 = ms_alignment2(p0,ppm=5)
timing_test("Dynamic Grouping with sub-grouping")
p3,bg_peaks = bg_removal(p_align_2,np.arange(0,184),ratio=1)
